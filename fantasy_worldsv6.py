...
def init():
    ...
    inst=Instructions_display(batch=inst_batch)
    window.push_handlers(inst.key_handler)
...
'''class Interstitial(Instructions_display):
    def __init__(self,*args,**kwargs):
        super(Interstitial,self).__init__(int_img1,*args,**kwargs)
...
'''def start_game():
    global game_objects,avatar,level
...
'''def pause_game():
    ...
    pyglet.clock.unschedule(update_boss_level)
    pyglet.clock.unschedule(change_direction)
    pyglet.clock.unschedule(create_hazards)
    pyglet.clock.unschedule(spawn_monster)'''
...
def set_level(level):
    ...
    global game_objects,avatar,boss_defeated_label,boss_hits_label
    ...
    if level==1:
        image_list=level_1_images
    if level==2:
        image_list=level_2_images
    else:
        image_list=level_1_images
...
init()
pyglet.clock.schedule_interval(check_instructions,1.0/120.0)
